  ...
  <div class="validator-wrap">
    ...
    <div class="mb-10" style="padding: 0 4px">
      <div style="width: 100%; line-height: 32px" flex="main:justify">
        <div>
          <f-checkbox v-model="isRequired" @change="requiredChange" style="margin-right: 0">必填项</f-checkbox>
          <f-divider type="vertical" />
          <f-dropdown @command="setRules">
          ...
          <f-dropdown @command="setRules">
            ...
            <template #dropdown>
              <f-dropdown-menu>
                <f-dropdown-item v-for="(name, key) in MULTIPLE_RULE" :key="key" :name="name">
                  {{ RULE_NAME_MAP[name] }}
                </f-dropdown-item>
              </f-dropdown-menu>
            </template>
        ...
        <div class="rules-params">
          <f-space>
            ...
            <div class="title-box" title="触发条件">
              ...
              <f-select v-model="checkRules[index].trigger" size="small" append-to-body @change="emitValue">
                <f-option label="失焦" value="blur"></f-option>
                <f-option label="改变" value="change"></f-option>
              </f-select>
            </div>
            <div class="title-box" title="取值类型">
              <span title="非数字类型选择字符">取值类型</span>
              <f-select v-model="checkRules[index].type" size="small" append-to-body @change="emitValue">
                <f-option label="字符" value="string"></f-option>
                <f-option label="数字" value="number"></f-option>
              </f-select>
            </div>
          </f-space>
        </div>
        <div class="rules-cfg">
          ...
          <f-space>
            ...
            <template v-if="[RULE.idCode, RULE.unifiedCode, RULE.orgInstCode, RULE.regNo].includes(rule.name)">
              ...
              <f-checkbox v-model="checkRules[index].ignoreCase" @change="emitValue">忽略大小写</f-checkbox>
            </template>
            <!--条件必填，条件必不填-->
            ...
            <template v-if="rule.name === RULE.timeBound">
              <div class="title-box" title="比较模式">
                <span>比较模式</span>
                <f-select v-model="checkRules[index].compareMode" append-to-body size="small" @change="emitValue">
                  <f-option value="gt" label="&gt;"></f-option>
                  <f-option value="ge" label="&ge;"></f-option>
                  <f-option value="lt" label="&lt;"></f-option>
                  <f-option value="le" label="&le;"></f-option>
                </f-select>
              </div>
              <div class="title-box" @drop="onDrop($event, index)" @dragover="allowDrop($event)">
                ...
                <f-input v-model.trim="checkRules[index].time" size="small" @change="emitValue"></f-input>
              </div>
            </template>
          </f-space>
        </div>
