<template>
  <div class="field-label-button">
    <f-button-group>
      <!-- Draggable button -->
      <f-button :type="variant" class="btn-draggable">
        <svg
          aria-hidden="true"
          role="img"
          ...
          <template v-if="field.headerAttributeFilter">
            ...
            <div class="dropdown-list">
              <div v-for="(header, index) in field.headers" :key="index" style="padding: 2px 10px;">
                <f-checkbox v-model="header.checked">{{ header.label }}</f-checkbox>
              </div>
            </div>
          </template>
          <f-divider style="margin: 8px 0;" v-if="field.headerAttributeFilter && field.valueFilter"></f-divider>
          <!-- Values filter -->
          <template v-if="field.valueFilter">
            ...
            <div class="dropdown-list">
              <div v-for="(item, index) in fieldValues" :key="index" style="padding: 2px 10px;">
                <f-checkbox v-model="item.checked" @change="handleCheckChange">
