  ...
  <div class="page-container" :class="{ 'in-scroll': innerScroll }" :style="pageStyle">
    <div class="page-header" v-if="!hideHeader">
      ...
      <slot name="header">
        <div class="page-header-inner">
          <div class="page-header-heading" v-if="!hideTitle">
            ...
            <f-popover trigger="hover" placement="right" width="" v-if="isBubbles">
              <span class="page-header-heading-title">
                {{ normalTitle }}
                <f-icon name="question-circle-fill"></f-icon>
              </span>
              <template #content>
                <div>{{ normalDesc }}</div>
              </template>
          ...
          <f-icon v-if="showClose" name="close" type="button" @click="$emit('close')"></f-icon>
        </div>
      </slot>
    </div>
    <div class="page-content" :style="{ margin: innerScroll ? 0 : contentMargin }">
      ...
      <div
        ...
        :class="{ 'has-bg': bg }"
      >
        <f-scrollbar wrap-class="scrollbar-box" always>
