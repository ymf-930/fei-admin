<template>
  <page-wrapper>
    <div class="preview-panel" flex>
      <div class="left-images">
        <div class="top-bar image-list">
          图片列表
          <f-tooltip class="item" content="点击图片载入其区域配置" placement="right">
            <f-icon name="question-circle"></f-icon>
          </f-tooltip>
        </div>
        <div class="image-list-wp">
          ...
          <f-empty v-if="listCfg.length === 0">暂无列表</f-empty>
        </div>
      </div>
      ...
      <div class="right-config" ref="contentRef">
        <div class="top-bar" flex="main:justify" v-if="renderer">
          <div>
            ...
            <f-button @click="checkConfig">查看配置</f-button>
          </div>
          <div>
            <f-button @click="saveConfig(true)">保存</f-button>
          </div>
        </div>
        ...
        />
        <f-empty v-if="listCfg.length === 0 && !renderer">请先准备可操作的图片</f-empty>
        ...
        <transition name="fade-right">
          <div class="config-panel" v-if="configModal">
            <div class="top-bar" flex="main:justify">
              ...
              <div>
                <f-button size="small" plain @click="closeConfigPanel">关闭</f-button>
                <f-button size="small" plain type="primary" @click="saveGroovy">保存脚本</f-button>
              </div>
            </div>
            <div class="edit-content" :style="{ height: height + 'px' }" flex>
              <div class="editor">
                <f-ace-editor v-model="configScript" lang="groovy" :height="height + 'px'" />
              </div>
              <div class="keys">
    ...
    <f-modal title="当前配置" v-model="configVisible" width="700px">
      <f-ace-editor :model-value="JSON.stringify(config, null, 2)" height="400px" />
    </f-modal>
  </page-wrapper>
</template>
