<template>
  <page-container>
    ...
    <f-collapse-wrap title="基础动态编码" shadow="none" class="mb-16">
      <div flex>
        <div class="p10" style="width: 40%">
          ...
          <div>
            字段候选
            <f-tag type="primary" draggable="true" @dragstart="dragField($event, 'name')">name</f-tag>
            <f-tag type="info" draggable="true" @dragstart="dragField($event, 'price')">price</f-tag>
            <f-tag type="success" draggable="true" @dragstart="dragField($event, 'num')">num</f-tag>
            <f-tag type="warning" draggable="true" @dragstart="dragField($event, 'discount')">discount</f-tag>
            <f-tag type="danger" draggable="true" @dragstart="dragField($event, 'breaks')">breaks</f-tag>
          </div>
          <f-divider style="margin: 10px 0"></f-divider>
          ...
          <f-button type="primary" @click="runTest1">执行计算</f-button>

          <f-divider style="margin: 10px 0"></f-divider>

          <f-alert type="success" v-if="fieldsTitle" show-icon>
            执行计算成功
        ...
        <div class="p10" style="width: 60%">
          <f-alert show-icon>
            <template #icon>
              <f-icon name="codelibrary"></f-icon>
            </template>
            计算函数
            <template #desc>
    ...
    <f-collapse-wrap title="表格类方式调用" shadow="none" class="mb-16">
      <div flex>
        <div class="p10" style="width: 70%">
          <f-table :columns="columns" :data="goodsData" border :merge-method="handleSpan" size="small"></f-table>
        </div>
        <div class="p10" style="width: 30%">
          ...
          <f-button type="primary" @click="runTest2">执行计算</f-button>
        </div>
      </div>
    </f-collapse-wrap>
    ...
    <f-collapse-wrap title="用于校验函数" shadow="none" class="mb-16">
      <div flex>
        ...
        <div class="p10" style="width: 30%">
          ...
          <f-space>
            <f-button size="mini" @click="addOne">新增校验字段</f-button>
            <f-tag :key="tag" v-for="tag in formParams" type="primary" closable @close="handleCloseTag(tag)">
              {{ tag }}
            </f-tag>
            <f-input
              style="width: 100px"
          ...
          <f-button type="primary" @click="runTest3">执行校验</f-button>
        </div>
        <div class="p10" style="width: 30%" v-if="formResult.type">
          <f-alert :type="formResult.type" show-icon>
