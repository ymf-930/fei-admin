<template>
  <f-card :bordered="false" class="card-panel" shadow="never" :body-style="{ padding: '0' }" divider="no">
    <template #header>
    ...
    <div class="items-wrap">
      ...
      <div class="link-wrap">
        <div class="link-item" v-for="link in linksStore.links" :key="link.link">
          <f-dropdown trigger="contextmenu" @command="linksStore.closeLink" placement="bottom-start">
            <a :href="link.link" :target="link.newTab ? '_blank' : '_self'">{{ link.text }}</a>
            <template #dropdown>
              <f-dropdown-menu>
                <f-dropdown-item :name="link.link" style="color: #ed4014">移除</f-dropdown-item>
              </f-dropdown-menu>
            </template>
          </f-dropdown>
        </div>
        <div class="link-item">
          <f-button type="primary" icon="plus" transparent size="mini" @click="handleAdd">添加</f-button>
        </div>
      </div>
    </div>
  </f-card>
  <f-modal v-model="visible" title="添加一个快捷链接">
    <f-form ref="ruleFormRef" :model="form" label-width="115px" class="mt-16" :rules="ruleValidate">
      <f-form-item prop="link" label="链接地址">
        <f-input v-model="form.link"></f-input>
      </f-form-item>
      <f-form-item prop="text" label="链接名称">
        <f-input v-model="form.text"></f-input>
      </f-form-item>
      <f-form-item prop="text" label="新标签页打开">
        <f-switch v-model="form.newTab">
    ...
    <template #footer>
      ...
      <f-button type="primary" @click="submitForm">确 定</f-button>
    </template>
  </f-modal>
</template>
